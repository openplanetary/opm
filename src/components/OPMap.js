...
class OPMap extends React.Component {
  ...
  render () {
    const { onMouseMove, results, mousePosLat, mousePosLng } = this.props
    let searchResults
    if (results.length > 0) {
    ...
    return (
      <React.Fragment>
        <p className='mouse-pos'>{mousePosLat}&deg;, {mousePosLng}&deg;</p>
        <Map center={position} zoom={this.state.zoom} maxBounds={this.state.maxBounds} maxBoundsViscosity={this.state.maxBoundsViscosity} onMouseMove={onMouseMove}>
...
OPMap.propTypes = {
  mousePosLat: PropTypes.string,
  mousePosLng: PropTypes.string,
